...
function createHorizontalGroup(parentGroup) {
    ...
    var centerVertical = sortedLayersByX.reduce(function(prev, curr, ind, arr) {
        return prev && (Math.floor(Math.abs(margins[ind].top - margins[ind].bottom)) < 1);
    }, Math.floor(Math.abs(padding.top - padding.bottom)) < 1);
...
function createVerticalGroup(parentGroup) {
    ...
    var centerHorizontal = sortedLayersByY.reduce(function(prev, curr, ind, arr) {
        return prev && (Math.floor(Math.abs(margins[ind].left - margins[ind].right)) < 1);
    }, Math.floor(Math.abs(padding.left - padding.right)) < 1);
...
function createSingleChildGroup(parentGroup) {
    ...
    if (Math.floor(Math.abs(parentGroup.frame().width() - parentArtboard.frame().width())) < 1 && Math.floor(Math.abs(parentGroup.frame().height() == parentArtboard.frame().height())) < 1) {
        orientation = "vertical";
    }
    if (Math.floor(Math.abs(padding.top - padding.bottom)) < 1) {
        ...
        if (Math.floor(Math.abs(padding.left - padding.right)) < 1) {
            gravity = "center";
            setProperty(getId(parentGroup), "gravity", gravity);
            if (!hasMatchHeight) {
                padding.top = 0;
    ...
    if (Math.floor(Math.abs(padding.left - padding.right)) > 40) {
        orientation = "horizontal";
        setProperty(getId(child), "align", "right");
        setProperty(getId(parentGroup), "paddingLeft", 0);
        hasRightAlign = true;
    }
    if (Math.floor(Math.abs(padding.top - padding.bottom)) > 50) {
        orientation = "vertical";
