...
function applyProp(element, attribute, set) {
  ...
  } else {} // Android.runInUI(parseParams("linearLayout", prop, "set"));
...
function getConfigGroups(config, type) {
  ...
  if (getSetType == "set") {
    ...
    if (!widthFound) groups.PARAMS.push({
      key: "width",
      value: 'wrap_content'
    });
    if (!heightFound) groups.PARAMS.push({
      key: "height",
      value: 'wrap_content'
    });
  }
...
function mashThis(attrs, obj, belongsTo, transformFn, allProps, type) {
  ...
  if (attrs.key == "fontStyle") {
    if (isURL(attrs.value)) {
      ...
      if (window.juspayAssetConfig && window.juspayAssetConfig.fonts && window.juspayAssetConfig.fonts[jpFont]) {
  ...
  if (attrs.key == "cursorColorV2") {
    var _id = allProps.find(function (a) {
      return a.key === "id";
    });
    ...
    if (_id != undefined) {
      ...
      prePend += ";set_currentView=this->findViewById:i_" + _id.value;
  ...
  if (attrs.key == "shimmer") {
    try {
      ...
      console.log(shimmerJson, shimmerJson.contents, attrs, attrs, obj, belongsTo, transformFn, allProps, type); // if(shimmerJson && shimmerJson.contents){
  ...
  if (!currTransVal && dontLoad) _cmd = "";else if (!prePend && !currTransVal) _cmd += appendArgs(attrs, obj) + ';';else _cmd += currTransVal + ';'; // for testing
      ...
      if (!globalObjMap.VIEW) {
        if (getSetType == "set") {
          if (type != "linearLayout") {
            groups.VIEW = groups.VIEW.filter(function (views) {
...
function addClickFeedback(rippleColor, disableFeedback, enableRadii) {
  ...
  if (enableRadii) {
    ...
    feedback += "set_arr=java.util.ArrayList->new;"; // 8 values, 2 for each corner. Starts at the left-top and moves clock-wise
