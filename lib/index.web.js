...
var helper = __webpack_require__(/*! ../helper */ "./src/helper.js");
var _require2 = __webpack_require__(/*! ./Utils */ "./src/WEB/Utils.js"),
...
var _require3 = __webpack_require__(/*! ./ListPresto */ "./src/WEB/ListPresto.js"),
  diffArray = _require3.diffArray;
var _require4 = __webpack_require__(/*! ./MapAttributes */ "./src/WEB/MapAttributes.js"),
  setUseHintColor = _require4.setUseHintColor;
var storedDiv = "";
var _require5 = __webpack_require__(/*! ./Utils */ "./src/WEB/Utils.js"),
  addToContainerList = _require5.addToContainerList,
  getContainer = _require5.getContainer,
  getParentView = _require5.getParentView,
  postRenderElements = _require5.postRenderElements;
...
function setPostRender() {
  if (postRenderElements.focusedElement) {
    if (state.hasAnimationProps > 0) {
      ...
      var elem = document.getElementById(postRenderElements.focusedElement);
      if (elem) {
        elem.focus();
      }
    }
  ...
  if (postRenderElements.selectAll) {
    var _elem = document.getElementById(postRenderElements.selectAll);
    if (_elem) {
      _elem.select();
    }
  ...
  if (postRenderElements.scrollToID) {
    var _elem2 = document.getElementById(postRenderElements.scrollToID);
    if (_elem2) {
      _elem2.scrollIntoView({
        behavior: "smooth",
        block: "nearest"
      });
    }
...
function Render(view, cb, namespace) {
  var useStoredDiv = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
...
function addViewToParent(id, view, index, cb, replace, namespace) {
  ...
  if (!parentElement || !parentView) {
    console.error("Parent element with id: ".concat(id, " not found"));
    return;
  }
...
function addEventListeners(eventListeners) {
  var _loop = function _loop() {
    view = eventListeners[i].view;
    props = view.props;
    id = eventListeners[i].id;
    var elem = document.getElementById(id);
    if (!elem) return "continue";
    var key = eventListeners[i].event;
...
function addProps(props) {
  ...
  if (props.hasOwnProperty("cornerRadii")) {
    var _props$cornerRadii$sp = props.cornerRadii.split(','),
      _props$cornerRadii$sp2 = _toArray(_props$cornerRadii$sp),
      radius = _props$cornerRadii$sp2[0],
      corners = _props$cornerRadii$sp2.slice(1);
    ele_style += "border-radius: " + corners.map(function (bool) {
      return "".concat(bool * radius, "px");
    }).join(' ') + ";";
  }
...
function addStrokeProp(props) {
  ...
  if (props.hasOwnProperty("stroke")) {
    ...
    if (values.length == 2) {
      ...
      var _iterator = _createForOfIteratorHelper(values[2]),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var i = _step.value;
          if (strokeMap[i] !== undefined) ele_style += strokeMap[i] + ": " + values[0] + "px solid " + values[1] + ";";
        }
      } catch (err) {
        _iterator.e(err);
      } finally {
...
function addLayout(elem, type, props) {
  ...
  var scrollBarVisible = props.hasOwnProperty('scrollBarVisible') ? props.scrollBarVisible : false; // by default, hiding the scrollbar, unless `scrollBarVisible` prop is used
  var elem_style = "";
  switch (type) {
    case 'linearLayout':
...
function addImage(type, props, elem) {
  if (type == 'imageView') {
    if (props.imageUrl || props.gifUrl) {
      var imageUrl = props.imageUrl || props.gifUrl;
      if (props.rawData) {
        // Do Nothing
      } else {
...
function getArticle(children, config) {
  ...
  if (config.isHtmlContent) article.innerHTML = config.textFromHtml;else article.textContent = config.text;
...
function patternCheck(props) {
  ...
  if (length && finalData.length > length) return false;
...
function addTextProperties(props, elem, type) {
  ...
  if (props.hasOwnProperty("text")) {
    ...
    } else elem_style += createTextElement(elem, props);
  }
...
function addTypeSpecificProp(type, props) {
  ...
  if (type == "editText") {
    if (props.hasOwnProperty("gravity")) ele_style += "text-align:" + props.gravity + ";";
...
function addSetInputTypeProp(ele, props) {
  ...
  if (props.hasOwnProperty("inputType")) {
    ...
    if (props.inputType == "numericPassword" || props.inputType == "password") {
      ...
      if (props.inputTypeI == 4 && window.innerWidth < 550 || navigator.userAgent.search("Firefox") === -1) {
...
function addPseudoClasses(elem, view) {
  ...
  if (props.hasOwnProperty("inputType")) {
    ...
    var boxShadowVal = "0 0 0 1000px " + props.background + " inset !important;";
  ...
  if (props.hasOwnProperty("hintColor") && useHintColor) {
    css += "input#\\3" + elem.id[0] + " " + elem.id.substring(1) + "::placeholder {color: ".concat(props.hintColor, " !important; }");
...
var setUseHintColor = function setUseHintColor(val) {
  return useHintColor = val;
};
module.exports = {
...
var _require = __webpack_require__(/*! ../compute */ "./src/compute.js"),
  computeChildDimens = _require.computeChildDimens;
var _require2 = __webpack_require__(/*! ./Utils */ "./src/WEB/Utils.js"),
  postRenderElements = _require2.postRenderElements,
  state = _require2.state;
...
function initiateElement(type, props, elem) {
  ...
  var events = ['onClick', 'onEnterPressedEvent', 'onDeletePressedEvent', 'onRightArrowPressedEvent', 'onLeftArrowPressedEvent', 'onChange', 'onMouseDown', 'onMouseUp', 'onMouseEnter', 'onMouseOver', 'onMouseMove', 'onMouseOut', 'onMouseLeave', 'onFocus', 'onPaste', 'getInputEventData', 'onAnimationEnd'];
  var _loop = function _loop() {
    var key = events[i];
    var eventType = key.substring(2, key.length).toLowerCase();
    if (props.hasOwnProperty(key) && typeof props[key] == "function") {
      ...
      if (eventType == "change") {
        ...
        if (eventType == "click") {
          var feedbackMutex = false;
          var touchFeedbackFn = function touchFeedbackFn(e) {
            if (feedbackMutex) return;
            feedbackMutex = true;
            var ogBg = "",
              temp = elem;
            while (ogBg === "") {
              if (temp.style.backgroundColor !== "") ogBg = temp.style.backgroundColor;
              temp = temp.parentNode;
            }
            var rippleColor = pSBC(-0.23, ogBg); // darken whatever the original background color was
            if (props.hasOwnProperty("rippleColor") && props.rippleColor.trim() !== "") rippleColor = props.rippleColor;
...
var pSBC = function pSBC(p, c0, c1, l) {
  ...
  if (!this.pSBCr) this.pSBCr = function (d) {
    ...
    if (n > 9) {
      var _d, _d2;
      (_d = d = d.split(","), _d2 = _slicedToArray(_d, 4), r = _d2[0], g = _d2[1], b = _d2[2], a = _d2[3]), n = d.length;
...
var InlineAnimationMapper = {
  ...
  MAPPINGS: {
    "from": {
      "fromX": function fromX(val) {
        return "transform: translateX(".concat(val, "px);");
      },
      "fromY": function fromY(val) {
        return "transform: translateY(".concat(val, "px);");
      },
      "fromScaleX": function fromScaleX(val) {
        return "transform: scaleX(".concat(val, ");\ntransform-origin:top left;");
      },
      "fromScaleY": function fromScaleY(val) {
        return "transform-origin:top left;\ntransform: scaleY(".concat(val, ");");
      },
      "fromRotation": function fromRotation(val) {
        return "transform: rotate(".concat(val, "deg);");
      },
      "fromRotationX": function fromRotationX(val) {
        return "transform: rotateX(".concat(val, "deg);");
      },
      "fromRotationY": function fromRotationY(val) {
        return "transform: rotateY(".concat(val, "deg);");
      },
      "fromAlpha": function fromAlpha(val) {
        return "opacity: ".concat(val, ";");
      }
    },
    "to": {
      "toX": function toX(val) {
        return "transform: translateX(".concat(val, "px);");
      },
      "toY": function toY(val) {
        return "transform: translateY(".concat(val, "px);");
      },
      "toScaleX": function toScaleX(val) {
        return "transform: scaleX(".concat(val, ");\ntransform-origin:top left;");
      },
      "toScaleY": function toScaleY(val) {
        return "transform-origin:top left;\ntransform: scaleY(".concat(val, ");");
      },
      "toRotation": function toRotation(val) {
        return "transform: rotate(".concat(val, "deg);");
      },
      "toRotationX": function toRotationX(val) {
        return "transform: rotateX(".concat(val, "deg);");
      },
      "toRotationY": function toRotationY(val) {
        return "transform: rotateY(".concat(val, "deg);");
      },
      "toAlpha": function toAlpha(val) {
        return "opacity: ".concat(val, ";");
      }
    },
    ...
    "animation-props": {
      "interpolator": function interpolator(val) {
        if (!val) {
          return "ease";
        } else if (typeof val == "string" && val.split(',').length == 4) {
          return "cubic-bezier(".concat(val, ")");
        } else {
          switch (val) {
            case "easein":
              return "ease-in";
      ...
      "repeatMode": function repeatMode(val) {
        switch (val) {
          case "restart":
            return "normal";
          case "reverse":
      ...
      "repeatCount": function repeatCount(val) {
        if (!val) return "1";
        val = parseInt(val);
        if (isNaN(val)) {
          return "1";
      ...
      "fillMode": function fillMode(val) {
        if (!val) return "forwards";else return "".concat(val);
      }
    }
  }
};
var CSSMarkupWriter = {
  "animations": {
    "keyframe": function keyframe(name, code) {
      return "@keyframes ".concat(name, " {").concat(code, "}");
    },
    "keyframe-from": function keyframeFrom(code) {
      return "from {".concat(code, "}");
    },
    "keyframe-to": function keyframeTo(code) {
      return "to {".concat(code, "}");
    },
    "animation-shorthand": function animationShorthand(keyframes) {
      var val = "";
...
function setAnimationStyles(elem, props) {
  ...
  try {
    var animationObjects = JSON.parse(props.inlineAnimation);
    ...
    animationObjects.forEach(function (animationObject) {
      var keyframeName = "keyframe_" + props.id + "_" + KEYFRAME_INDEX++;
      /* Add keyframe in css */
      for (var _i2 = 0, _Object$entries = Object.entries(animationObject); _i2 < _Object$entries.length; _i2++) {
      ...
      var keyFrameCSS = AnimationCSSMarkupWriter["keyframe"](keyframeName, AnimationCSSMarkupWriter["keyframe-from"](keyFrameFromMarkup) + AnimationCSSMarkupWriter["keyframe-to"](keyFrameToMarkup));
      addCSSStyle(keyFrameCSS);
      window.__RENDERED_KEYFRAMES.push(keyframeName);

      /* Add animation shorthand prop of keyframe in element*/
      var keyFrameAnimShorthand = "".concat(keyframeName, " ");
      ...
      if (animationObject.repeatCount != "-1") {
        ...
        elem.onanimationend = function () {
          ...
          if (state.hasAnimationProps == 0) manualFocus();
        };
      }
      ;
    });
...
var renderList = function renderList(view, elem, computeList) {
  ...
  if (view.props.diffArray == "filter") {
    ...
    for (var _i3 = 0; _i3 < view.props.itemDatas.length; _i3++) {
      var chrome50matchList;
      if (isChrome50()) {
        chrome50matchList = {
          h: [],
  ...
  //replacing only the diff elements
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        var oldView = view.children[_i4];
        var oldChild = document.getElementById(oldView.props.id);
        if (oldChild) {
          var newChildView = List.getChildItemViewByIndex(view, _i4);
          newChildView.onClick = oldView.onClick;
          view.children[_i4] = newChildView;
          computeChildDimens(view);
          var chrome50matchList;
          if (isChrome50()) {
            chrome50matchList = {
...
function handleMatchParentChrome50(chrome50matchList) {
  ...
  for (var x = chrome50matchList.h, i = 0; x && i < x.length; ++i) {
    if (x[i] && x[i].props && x[i].props.id) {
      ...
      && elm.parentElement.parentElement.style && elm.parentElement.parentElement.style.flexDirection == "column" && elm.parentElement.style['flex-grow'] == "1" && elm.parentElement.style['flex-shrink'] == "1"
  ...
  for (var x = chrome50matchList.w, i = 0; x && i < x.length; ++i) {
    if (x[i] && x[i].props && x[i].props.id) {
      var elm = document.getElementById(x[i].props.id);
      if (elm && elm.parentElement && elm.parentElement.parentElement && elm.parentElement.parentElement.style
      // Code to handle cases where parent is width 0 weight 1
      // and parent of parent is orientation horizontal
      && elm.parentElement.parentElement.style.flexDirection == "row" && elm.parentElement.style['flex-grow'] == "1" && elm.parentElement.style['flex-shrink'] == "1"
...
function getValueFromPixel(pixel) {
  return parseInt(pixel.substring(0, pixel.length - 1));
}
;
...
function getContainer(namespace) {
  if (namespace) {
    ...
    if (container) return container;else return undefined;
  }
...
module.exports = {
  ...
  postRenderElements: postRenderElements,
  state: state
};
...
function computeRelativeLayout(view) {
  ...
  children.forEach(function (child2) {
    var child = window.__VIEWS[child2.props.id] || child2;
...
function parseLayoutProps(type, config, key) {
  ...
  if (key == "cornerRadii") {
    var _config$cornerRadii$s = config.cornerRadii.split(','),
      _config$cornerRadii$s2 = _toArray(_config$cornerRadii$s),
      radius = _config$cornerRadii$s2[0],
      corners = _config$cornerRadii$s2.slice(1);
    config.style.borderRadius = corners.map(function (bool) {
      return "".concat(bool * radius, "px");
    }).join(' ') + ";";
  }
  if (key == "alpha") {
  ...
  if (key == "stroke") {
    ...
    if (_values.length == 2) config.style.border = _values[0] + "px solid " + _values[1];else if (_values.length == 3) {
      ...
      var _iterator = _createForOfIteratorHelper(_values[2]),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var i = _step.value;
          if (strokeMap[i] !== undefined) config.style[strokeMap[i]] = _values[0] + "px solid " + _values[1];
        }
      } catch (err) {
        _iterator.e(err);
      } finally {
